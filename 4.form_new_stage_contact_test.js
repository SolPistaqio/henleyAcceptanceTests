...
Scenario("test form", async ({ I }) => {
  ...
  for (let i = 1; i < faOptions.length; i++) {
    ...
    I.fillField("У меня есть вопрос*", title);
    ...
    I.fillField("Имя*", "СОЛ_ТЕСТ contact");
    I.fillField("Фамилия*", faOptions[i]);
    I.click(locate(".ui-selectmenu-text").at(5));
    for (let a = 0; a < i; a++) {
    ...
    if (isOdd(i)) {
      I.click(locate(".ui-selectmenu-text").at(6));
      for (let a = -1; a < i; a++) {
        I.pressKey("ArrowDown");
      }
      I.pressKey("Enter");
    ...
    I.click(locate(".ui-selectmenu-text").at(7));
    for (let a = 0; a < i; a++) {
      I.pressKey("ArrowDown");
    }
